...
const embedded = process.binding('embedded');
...
Module.prototype.load = function(filename) {
  ...
  if (Module.globalPathsOnly) {
    var acceptableRoots = [];
    for (var i = 0; i < Module.globalPaths.length; ++i) {
      acceptableRoots.push(path.resolve(Module.globalPaths[i]));
    }

    var filteredPaths = [];
    for (var i = 0; i < this.paths.length; ++i) {
      for (var j = 0; j < acceptableRoots.length; ++j) {
        if (this.paths[i].startsWith(acceptableRoots[j])) {
  ...
  if (Module.isEmbedded(filename)) extension = '.embedded';
...
// Native extension for .embedded
Module._extensions['.embedded'] = function(module, filename) {
  var content = embedded[filename];
  module._compile(internalModule.stripBOM(content), filename);
};
...
Module.isEmbedded = function(module) {
  return module.startsWith('embedded/');
}
...
Module._initPaths = function() {
  ...
  // [adill] expose globalPaths as mutable instead
